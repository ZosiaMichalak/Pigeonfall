...
// Rounds coordinates to prevent subpixel rendering blur.
static inline float px(float v) { return std::floor(v + 0.5f); }

// Constructor: links font.
DifficultySelectUI::DifficultySelectUI(sf::Font& font) : font_(font) {}

// Displays the picker and sets default selection to NORMAL.
...
// Closes and hides the picker.
void DifficultySelectUI::close() {
    open_             = false;
}

// Checks key presses: updates selected index on Left/Right and registers chosen difficulties on Enter/E.
SlotUIResult DifficultySelectUI::handleEvent(const sf::Event& event) {
    ...
    // Traverse indices: 0 = EASY, 1 = NORMAL, 2 = HARD
...
// Renders the background dim overlay, centered window frame, buttons, labels, and footer hint.
void DifficultySelectUI::render(sf::RenderWindow& window) {
    if (!open_) return;

    // Full screen dim overlay (translucent black)
    sf::RectangleShape dim({ VIEW_W, VIEW_H });
    dim.setFillColor(sf::Color(0, 0, 0, 160));
    window.draw(dim);

    // Centered pop-up panel container
    ...
    // Difficulty button config structures
    ...
    // Draw EASY, NORMAL, and HARD buttons in a row
    for (int i = 0; i < 3; ++i) {
        ...
        // Highlight active button outline with its difficulty color
    ...
    // Interactive controls footer hint
...
// Renders the background panel rectangle box.
...
// Generates and positions text nodes.
